...
# UDP config
CONTROL_HUB_IP   = "192.168.43.1"  # default Control Hub IP
UDP_PORT         = 9000
...
def send_values(lx, ly, rx, ry):
    # Pack 4 floats into 16 bytes and send over UDP
    data = struct.pack('ffff', lx, ly, rx, ry)
    sock.sendto(data, (CONTROL_HUB_IP, UDP_PORT))
...
while cap.isOpened():
    ...
    # Send values over UDP every frame
    send_values(left_jx, left_jy, right_jx, right_jy)
